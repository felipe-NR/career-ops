...
17. Build the render payload (see the **JSON Input Schema** below) from the tailored content — emit compact structured JSON, **not** full HTML markup — and write it to `/tmp/cv-{candidate}-{company}-{report}.json` when a report exists, or `/tmp/cv-{candidate}-{company}.json` only for a truly report-less one-off. The report-scoped path prevents parallel roles at the same company from overwriting each other's payload.
18. Run `node build-cv-html.mjs {payload-path} {html-path} {template}`, where `{html-path}` is the active bundle's `cv/tailored/vNNN/cv.html`, `output/cv-{candidate}-{company}-{report}.html` when a report exists, or `output/cv-{candidate}-{company}.html` only for a report-less one-off CV, and `{template}` is the path printed by **Selecting the template** below (omit it to use the base template). The report number is mandatory in flat HTML/PDF artifact names whenever known. The script owns every tag, CSS class, and HTML escaping. Keep the HTML outside temporary storage because the dashboard's `D` hotkey regenerates from it.
...
21. Execute: `node generate-pdf.mjs {html-path} {pdf-path} --format={letter|a4} --report={report number}`, where `{pdf-path}` is the active bundle's `cv/tailored/vNNN/cv.pdf`, `output/cv-{candidate}-{company}-{report}-{YYYY-MM-DD}.pdf` when a report exists, or `output/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf` only for a report-less one-off CV. `{report number}` is the NNN from the report filename/link (e.g. `008` for `reports/008-acme-….md`), not the tracker `#` column. Pass it whenever the application has (or will have) a report; it records the PDF↔report linkage in `data/pdf-index.tsv` so the dashboard can open and regenerate the exact nested or flat HTML/PDF pair. `generate-pdf.mjs` refuses concurrent writers to one destination and refuses to overwrite a path already owned by another report. Omit `--report` only for one-off CVs with no tracker entry.
