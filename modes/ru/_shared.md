...
|------------|------------|
| MCP fetch (`fetch`) | **ИНСТРУМЕНТ ПО УМОЛЧАНИЮ** для веб-доступа. Использовать для ВСЕХ URL — извлечение JD, страницы компаний, данные о компенсации, верификация. Конвертирует HTML в markdown. Доступен во всех средах. |
| WebSearch | Исследование компенсации, тренды, культура компании, контакты в LinkedIn — только когда нужен ПОИСК (нет известного URL). |
| Playwright | Fallback для JS-тяжёлых SPA, которые MCP fetch не может отрендерить. Также для генерации PDF. **НИКОГДА 2+ агентов с Playwright параллельно.** |
...
- **MCP fetch — основной инструмент.** Используйте `fetch` для всех URL — без ограничений на количество запросов, работает одинаково на всех моделях.
...
- Замените каждую инструкцию WebSearch в файлах режимов на MCP fetch с конкретными известными URL:
  - Данные о компенсации → `fetch` `https://www.levels.fyi/company/{company}/` или `https://www.glassdoor.com/Overview/company-{slug}.htm`
  - Информация о компании → `fetch` страницы вакансий или страницы «О компании»
  - Извлечение JD → `fetch` URL вакансии напрямую (поиск не нужен)
  - Контакты LinkedIn → Спросить кандидата об именах, затем `fetch` URL профиля
  - Новости об увольнениях → `fetch` известных новостных источников, если URL доступен, иначе пропустить
- Цепочка fallback конвейера: **MCP fetch → Playwright** (2 уровня, без уровня WebSearch)
- Scan Уровень 3 (обнаружение через WebSearch) **недоступен**. Добавляйте компании вручную в `tracked_companies` в `portals.yml`.

**Если вы используете Anthropic модель (Claude):** MCP fetch остаётся инструментом по умолчанию для URL-доступа. Используйте WebSearch только для широкого поиска, когда нет конкретного URL.
