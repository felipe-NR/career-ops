...
### プラットフォームルール（Anthropic 以外のモデル）

**Anthropic 以外のモデル（GLM、Gemini、DeepSeek、Qwen など）で実行している場合：**
- **内蔵の WebSearch ツールを使用しない。** Anthropic 以外のモデルは厳しい WebSearch レート制限があり、career-ops がすぐに使い切ってしまう。クォータを温存する。
- モードファイル内のすべての WebSearch 指示を、具体的な既知の URL を使った WebFetch に置き換える：
  - 報酬データ → WebFetch `https://www.levels.fyi/company/{company}/` または `https://www.glassdoor.com/Overview/company-{slug}.htm`
  - 企業情報 → 企業の採用ページや会社概要ページを WebFetch
  - JD 抽出 → JD URL を直接 WebFetch（検索不要）
  - LinkedIn コンタクト → 候補者に名前を尋ね、プロフィール URL を WebFetch
  - レイオフニュース → URL があれば既知のニュースソースを WebFetch、なければスキップ
- パイプラインのフォールバックチェーンは **Playwright → WebFetch** の 2 段階に（WebSearch 段階なし）
- スキャンレベル 3（WebSearch ディスカバリー）は **利用不可**。企業は `portals.yml` の `tracked_companies` に手動で追加すること。

**Anthropic モデル（Claude）で実行している場合：** ドキュメント通りに WebSearch を使用。すべてのモードファイルはそのまま機能する。
