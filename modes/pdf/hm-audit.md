...
4. **Tailored bullets** — resolve the artifact for *this* role, in order. Never parse the `.pdf`.
   1. `/tmp/cv-{candidate}-{company}-{report}.json` from the same session — the cleanest source (`experience[].bullets[]`). Use the report-less legacy path only for a true one-off with no report. This is the usual case, since the audit runs in the same `pdf` flow that wrote it.
