...
/**
 * Identify one scan run. Threaded into every history row the run writes so an
 * agent can recover exactly the URLs a completed run produced, which a plain
 * date cannot do once two runs share a day.
 */
export function createScanRunId(now = new Date()) {
  return `scan-${now.toISOString().replace(/[-:.]/g, '')}-${randomUUID().slice(0, 8)}`;
}

/**
 * Widen an existing TSV header with append-only columns it does not name yet.
 * Data rows are never rewritten: every column added here goes at the END, so
 * older rows simply stop short and positional readers are unaffected.
 */
...
export function formatScanHistoryRow(offer, date, status = 'added', runId = '') {
  ...
  // Optional so callers outside scan.mjs keep byte-compatible rows. The primary
  // scanner always supplies one.
...
export async function appendToScanHistory(offers, date, status = 'added', runId = '') {
  await withPipelineLock(SCAN_HISTORY_PATH, () => {
    ...
    if (!existsSync(SCAN_HISTORY_PATH)) {
      ...
      atomicWriteFile(SCAN_HISTORY_PATH, 'url\tfirst_seen\tportal\ttitle\tcompany\tstatus\tlocation\tfingerprint\tposted_at\ttrust_score\ttrust_flags\tnormalized_company\trun_id\n');
    } else if (runId) {
...
export function appendScanRunSummary(c, filePath = SCAN_RUNS_PATH) {
  ...
  const row = [
    ...
    // `run_id` identifies the exact scan-history rows this run created.
    c.runId ?? '',
  ].join('\t') + '\n';
...
async function main() {
  ...
  // One id per run, minted before anything is written, so the history rows and
  // the scan-runs summary row of this run all carry the same key.
  ...
  if (!dryRun) {
    registerRunFailureSnapshot(() => ({
      timestamp: new Date().toISOString(), runId,
  ...
  if (!dryRun) {
    ...
    appendScanRunSummary({
      timestamp: new Date().toISOString(), status: 'completed', runId,
