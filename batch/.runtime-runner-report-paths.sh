...
PROMPT_FILE="$BATCH_DIR/.runtime-prompt-report-paths.md"
...
process_offer() {
  ...
  # Runtime-only pre-extraction for this pipeline run. Codex workers have MCP
  # disabled, so give each one a browser-rendered JD. The directory lock keeps
  # Playwright strictly serial even when two evaluation workers run in parallel.
  local browser_lock="$BATCH_DIR/.runtime-browser-extract.lock"
  while ! mkdir "$browser_lock" 2>/dev/null; do sleep 1; done
  local extraction_json=""
  extraction_json=$(node "$PROJECT_DIR/browser-extract.mjs" "$url" --mode jd --max-chars 30000 --timeout 30000 2>/dev/null || true)
  rmdir "$browser_lock" 2>/dev/null || true
  if [[ -n "$extraction_json" ]]; then
    printf '%s' "$extraction_json" | node -e '
      const fs = require("fs");
      let data = "";
      ...
      process.stdin.on("end", () => {
        try {
          ...
          if (typeof obj.text === "string" && obj.text.trim()) fs.writeFileSync(process.argv[1], obj.text);
        } catch {}
      });
    ' "$jd_file"
  fi
