...
process_offer() {
  ...
  local -a worker_args=()
  if [[ "$WORKER_CLI" == "claude" ]]; then
    # Keep this array's name stable: --strict-mcp-config is the #506 isolation
    # guarantee, and test-all.mjs asserts it by locating the Claude argument
    # array by name. Renaming it hides a real regression from that check.
