...
11. Write HTML to `output/cv-candidate-{company-slug}-{{REPORT_NUM}}.html`. The report number is mandatory in both HTML and PDF paths so sibling roles at the same company cannot overwrite one another.
12. Run:
