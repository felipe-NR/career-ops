...
try {
  ...
  // 31. Codex user-level config.toml with Playwright → detected.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-mcp-31-'));
    const codexHome = mkdtempSync(join(tmpdir(), 'co-codex-home-31-'));
    try {
      ...
      const state = runDoctor(dir, ['--cli', 'codex'], { CODEX_HOME: codexHome });
      if (!expectWarn(state, '#31 codex user config')) {
        // already failed
      } else if (state.active_cli === 'codex'
          && state.cli_source === 'flag'
  ...
  // 32. Codex project-scoped .codex/config.toml with Playwright → detected.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-mcp-32-'));
    const codexHome = mkdtempSync(join(tmpdir(), 'co-codex-home-32-'));
    try {
      ...
      const state = runDoctor(dir, ['--cli', 'codex'], { CODEX_HOME: codexHome });
      if (!expectWarn(state, '#32 codex project config')) {
        // already failed
      } else if (state.active_cli === 'codex'
          && state.cli_source === 'flag'
          && state.playwright_mcp?.codex === true
  ...
  // 33. A configured but explicitly disabled server is not operational and
  //     must not suppress the setup warning.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-mcp-33-'));
    const codexHome = mkdtempSync(join(tmpdir(), 'co-codex-home-33-'));
    try {
      writeFileSync(join(codexHome, 'config.toml'), [
        '[mcp_servers.playwright]',
        'command = "npx"',
