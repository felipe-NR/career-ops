 ...
 */
/**
 * Claim `lockPath` as a directory (mkdir is atomic across processes), run
 * `body`, and release it. Batch runs render several CVs concurrently, so the
 * read-modify-write of the PDF manifest below is a genuine race: two workers
 * that read the same `lines` both write, and the second silently drops the
 * first one's row. A lock older than `staleMs` is treated as abandoned, so a
 * killed worker cannot wedge every later render.
 */
...
/**
 * Refuse to rewrite a PDF path that the manifest already attributes to a
 * DIFFERENT report. Without this, two reports pointed at the same output
 * filename overwrite each other's CV and the tracker keeps linking both rows
 * to whichever render happened to run last.
 */
...
function updatePDFManifest(reportNum, pdfPath, htmlPath, format) {
  ...
  // O lock mora ao lado do manifesto, entao o diretorio precisa existir antes
  // do mkdirSync atomico -- caso contrario a reivindicacao falha com ENOENT.
  ...
  const normKey = normalizeReportKey;

  return withDirectoryLock(manifestLock, () => {
...
async function generatePDF() {
  ...
  // Refuse a concurrent render of the SAME output path. Batch runs drive
  // several workers at once, and two of them pointed at one filename produce a
  // torn PDF plus a manifest row that describes neither. The lock lives beside
  // the PDF, so its parent directory has to exist before mkdirSync can claim it
  // atomically -- renderHtmlToPdf creates that directory too, but only later.
  ...
  try {
    ...
    if (err?.code === 'EEXIST') {
      throw new Error(
        `Refusing concurrent PDF write: ${outputPath} is already being generated. `
        + 'Use a report-scoped filename.',
      );
    }
    throw err;
  }
  ...
  try {
    // Ownership is checked BEFORE the render, not just inside updatePDFManifest:
    // the manifest is written after the PDF, so a guard that only lives there
    // fires once the other report's file has already been overwritten.
    assertPDFPathOwnership(
      resolvePdfIndexPath(trackerPath),
      reportNum,
      relative(workspaceRoot, outputPath).split(sep).join('/'),
    );
