...
Встроенный терминальный дашборд для визуального просмотра пайплайна. Соберите один раз, запускайте в любое время — `go` уже в PATH, если вы используете `nix develop`.
